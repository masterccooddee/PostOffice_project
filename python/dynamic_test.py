...
def display_progress(mg):
    lineCounter = 0
    p_time = 0
    while True:
        for i in range(len(mg)):
            if lineCounter >= 5 and p_time == 0:
                print("\033[s\033[20B\033[u", end="")
                lineCounter = 0
                p_time += 1

            print(f"ID: {i}")
            print(f"{mg[i].get('Message1', '')}                                   ")
            print(f"{mg[i].get('Message2', '')}                                    ")
            if i != len(mg) - 1:
                print("\033[38;2;235;118;65m+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-\033[0m")

            lineCounter += 4

        up = len(mg) * 3 + len(mg) - 1
...
def sqa(start, pfs_v, iter_count, try_max):
    ...
    for iteration in range(iter_count):
        ...
        if current_cost < shortest_time:
            ...
            print(f"Updated Penalty: {temperature}")
        else:
