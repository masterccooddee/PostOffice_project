...
import multiprocessing  # Ensure this is included
    ...
    for i in range(iter_count):
        ...
        if l_time_cs < s_time_cs:
            ...
            try:
                ...
                if exponent > 700:  # or whatever threshold you prefer
                    exponent = 700
                elif exponent < -700:
                    exponent = -700
...
def main():
    ...
    seed1 = random.randint(0, int(1e6))  # Convert 1e6 to int
    seed2 = random.randint(0, int(1e6))  # Convert 1e6 to int
    ...
    with multiprocessing.Pool(processes=4) as pool:  # Set the number of processes
        seeds = [seed1, seed2]  # Add more seeds as necessary
